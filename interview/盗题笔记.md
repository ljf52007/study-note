...
首先,不可能把几万个`DOM`一次性插入,这样做是绝对会卡顿的,解决问题的关键应该是如果分批渲染`DOM`.

1. 通过`requestAnimationFrame`(`RAF`)的方式循环插入`DOM`.

   

2. 虚拟滚动`virtualized scroller`,这种技术的原理就是只渲染可视区域内的内容,非可见区域的就完全不渲染.当用户滚动的时候,实时替换渲染的内容.
