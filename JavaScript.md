...
比如，上文提到了`JS`的引擎线程和渲染线程，这两个线程是互斥的，`JS`运行时可能会阻止`UI`渲染。这其中的原因是因为`JS`可以修改`DOM`，如果在`JS`执行的时候`UI`线程也在工作，就可能导致不能安全的渲染`UI`。假定`JavaScript`同时有两个线程，一个线程在某个`DOM`节点上添加内容，另一个线程删除了这个节点，这时浏览器应该以哪个线程为准？
